...
// The PTZOptics VISCA over IP commands can be issued as TCP (Port 5678) or UDP (Port 1259) to the camera and are always issued as HEX.
#define TCP_ONLY 1
#ifdef VISCA_WIN
char *ttydev = "COM1:";
#else
...
{
    fprintf(stderr, "Usage: visca-cli [-d <tcp address>] command\n");
#if TCP_ONLY
    fprintf(stderr, "  udp and serial port are disabled\n");
#else
    fprintf(stderr, "  default serial port device: %s\n", ttydev);
#endif
    fprintf(stderr, "  for available commands see sourcecode or run 'strings' on the executable\n");
    exit(1);
}
...
void open_interface(void)
{
    ...
    // If I meant for someone else to use this, I'd come up with a better way of telling tcp/udp/serial apart. The ports are standard, no need to pass them on the command line. Maybe something like -d -u -t
#if TCP_ONLY
    int port = 5678;
    ...
    char *sep;
    // The PTZOptics VISCA over IP commands can be issued as TCP (Port 5678) or UDP (Port 1259) to the camera and are always issued as HEX.
    // basically, the options are 'udp://ip_address:', just 'ip_address:' for tcp, and anything else is serial. Colon required, because C strings. Ick. But at least you don't have to provide the port.
    // Also if you're on Windows, surprise! It's going to see the colon in 'COM1:' and oops, it's checking for tcp. Soooo... the #if.
    // Hey, I may write hacky code sometimes, but I document the hell out of it when I do.
    if (strncmp(ttydev, "udp://", 6) == 0 && (sep = strchr(ttydev + 6, ':'))) {
        int port = atoi(sep + 1) ?: 1259;
...
//    }
    ...
    fprintf(stderr, "Camera %s initialisation successful.\n", ttydev);
}
...
VISCA_API uint32_t backupRestore(VISCAInterface_t *iface, VISCACamera_t *camera, uint32_t inOffset, bool isBackup)
{
...
VISCA_API uint32_t backupRestoreOne(VISCAInterface_t *iface, VISCACamera_t *camera, uint32_t inOffset, uint32_t sceneIndex, bool isBackup)
{
    uint32_t fromOffset = isBackup ? 0 : inOffset;
    uint32_t toOffset = isBackup ? inOffset : 0;
...
static int doCommand(int argc, char **argv, int *ret1, int *ret2, int *ret3)
{
    ...
    if (arg1 != NULL) {
        intarg1 = (int)strtol(arg1, NULL, 0);
    }
    if (arg2 != NULL) {
        intarg2 = (int)strtol(arg2, NULL, 0);
    }
    if (arg3 != NULL) {
        intarg3 = (int)strtol(arg3, NULL, 0);
    }
    if (arg4 != NULL) {
        intarg4 = (int)strtol(arg4, NULL, 0);
    }
    if (arg5 != NULL) {
        intarg5 = (int)strtol(arg5, NULL, 0);
    }
    ...
    if (strcmp(command, "memory_set") == 0) {
        if ((arg1 == NULL) || (intarg1 < 0) || (intarg1 > VISCA_MEMORY_MAX)) {
            return 41;
        }
    ...
    if (strcmp(command, "scene_restore") == 0) {
        if (arg1 == NULL) {
            fprintf(stderr, "%s: [offset] restores scenes 1-9 from the given offset\n", command);
            return 10;
        }
        if ((intarg1 < 10) || (intarg1 > 110)) {
            // My camera goes to 127. Your camera might not.
            fprintf(stderr, "%s: offset (%d) must be in the range 10 to 110\n", command, intarg1);
            return 41;
        }
        if (backupRestore(&iface, &camera, intarg1, RESTORE) != VISCA_SUCCESS) {
            return 46;
        }
        return 10;
    }

    if (strcmp(command, "scene_backup") == 0) {
        if (arg1 == NULL) {
            fprintf(stderr, "%s: [offset] backs up scenes 1-9 to the given offset\n", command);
            return 10;
        }
        if ((intarg1 < 10) || (intarg1 > 110)) {
            // My camera goes to 127. Your camera might not.
        ...
        if (backupRestore(&iface, &camera, intarg1, BACKUP) != VISCA_SUCCESS) {
            return 46;
        }
        return 10;
    }

    if (strcmp(command, "scene_restore_one") == 0) {
        if (arg1 == NULL) {
            fprintf(stderr, "%s: [offset scene] restores the given scene (1-9) from the given offset\n", command);
            return 10;
        }
        if ((intarg1 < 10) || (intarg1 > 110)) {
            fprintf(stderr, "%s: offset (%d) must be in the range 10 to 110\n", command, intarg1);
        ...
        if ((arg2 == NULL) || (intarg2 < 1) || (intarg2 > 9)) {
            fprintf(stderr, "%s: scene (%d) must be in the range 1 to 9\n", command, intarg1);
            return 41;
        }
        if (backupRestoreOne(&iface, &camera, intarg1, intarg2, RESTORE) != VISCA_SUCCESS) {
            return 46;
        }
        return 10;
    }

    if (strcmp(command, "scene_backup_one") == 0) {
        if (arg1 == NULL) {
            fprintf(stderr, "%s: [offset scene] backs up the given scene (1-9) to the given offset\n", command);
            return 10;
        }
        if ((intarg1 < 10) || (intarg1 > 110)) {
            fprintf(stderr, "%s: offset (%d) must be in the range 10 to 110\n", command, intarg1);
        ...
        if (backupRestoreOne(&iface, &camera, intarg1, intarg2, BACKUP) != VISCA_SUCCESS) {
            return 46;
        }
        return 10;
    }
    ...
    if (strcmp(command, "set_pantilt_relative_position") == 0) {
        ...
        if (VISCA_set_pantilt_relative_position(&iface, &camera, intarg1, intarg2, intarg3, intarg4) !=
            VISCA_SUCCESS) {
            return 46;
        }
        return 10;
    ...
    if (strcmp(command, "set_date_time") == 0) {
        ...
        if (VISCA_set_date_time(&iface, &camera, intarg1, intarg2, intarg3, intarg4, intarg5) !=
            VISCA_SUCCESS) {
            return 46;
        }
        return 10;
